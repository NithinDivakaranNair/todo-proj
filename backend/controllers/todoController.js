...
export const updateTodo = async (req, res) => {
    try {
        ...
        if (todo.isDeleted) return res.status(400).json({ message: "Cannot update a deleted todo" });

        if (title === undefined && discription === undefined && status === undefined) {
            return res.status(400).json({
                success: false,
                message:"At least one field (title, discription, status) must be provided",
            });
        }
